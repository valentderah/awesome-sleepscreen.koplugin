...
function M.build(params, ctx)
    ...
    local date_size = tonumber(params.date_size) or 15
    local time_size = tonumber(params.time_size) or 40
    if WidgetSpan.col_span(ctx) >= 2 then
        date_size = math.floor(date_size * 1.12 + 0.5)
        time_size = math.floor(time_size * 1.12 + 0.5)
    end
