--[[ Localization for sleepscreenwidgets plugin (en msgids + ru + zh_CN .po). ]]
...
function M.load()
    ...
    local path = plugin_root .. "/l10n/" .. sub .. "/sleepscreenwidgets.po"
    local map = parse_po(path)
    if map and next(map) then
        translations = map
        local n = 0
        for _ in pairs(map) do n = n + 1 end
        logger.info("sleepscreenwidgets l10n: loaded " .. path .. " (" .. n .. " strings)")
    end
end
